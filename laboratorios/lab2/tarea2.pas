...
{   
    Aplica el formato `tfmt` a los caracteres de `ln` entre las columnas `ini` y `fin`, incluídos los extremos. Si todos los carácteres ya tienen el tipo de formato `tfmt`, en lugar de aplicarlo lo quita.
...
{   
    Busca la primera ocurrencia de la cadena `c` en la línea `ln` a partir de la columna `desde`. Si la encuentra, retorna en `pc` la columna en la que incia. 
...
{   
    Busca la primera ocurrencia de la cadena `c` en el texto `txt` a partir de la posición `desde`. Si la encuentra, retorna en `pp` la posición en la que incia. La búsqueda no encuentra cadenas que ocupen más de una línea.
...
var
    ...
    pc : PosibleColumna;
begin
    aux := ubicarLineaEnTexto(txt, desde.linea);
    contador := desde.linea;
    buscarCadenaEnLineaDesde(c, aux^.info, desde.columna, pc);
    aux := aux^.sig;
    while (aux <> nil) and (not pc.esColumna) do begin
        contador := contador + 1;
        buscarCadenaEnLineaDesde(c, aux^.info, 1, pc);
        if not pc.esColumna then 
            aux := aux^.sig
    end;
    pp.esPosicion := (aux <> nil);
